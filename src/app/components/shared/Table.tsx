  ...
  <table>
    ...
    <tbody>
      {tableData.map((dataObj) => (
        <tr key={crypto.randomUUID()} style={{ borderBottom: 'solid #d3d3d3' }}> {/** Use key generator since data can contain arrays */}
          {Object.keys(dataObj).map((datumKey, ind) => {
            const datumVal = dataObj[datumKey]
            const datumIsArr = Array.isArray(datumVal)
            return (
              <td key={datumIsArr ? datumKey : datumVal.toString()}> {/** Cast value as string in case `datumVal` is type boolean */}
                {datumIsArr
                ? datumVal.map(item => (
                  <p>{item}</p>
                ))
